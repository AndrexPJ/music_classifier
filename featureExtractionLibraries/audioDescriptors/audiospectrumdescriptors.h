...
public:
    SpCentroidDescriptorExtractor(AudioSpectrum<double> &spectrum, int result_frames_count = 1);
...
protected:
    virtual double extractForOneFrame(int channel_number, int frame_number);
public:
    SpFlatnessDescriptorExtractor(AudioSpectrum<double> &spectrum, int result_frames_count = 1);
    AudioDescriptorExtractor* clone() const;
};
...
protected:
    virtual double extractForOneFrame(int channel_number, int frame_number);
public:
    SpFluxDescriptorExtractor(AudioSpectrum<double> &spectrum, int result_frames_count = 1);
    AudioDescriptorExtractor* clone() const;
};
...
protected:
    virtual double extractForOneFrame(int channel_number, int frame_number);
public:
    SpRollOffDescriptorExtractor(AudioSpectrum<double> &spectrum, int result_frames_count = 1, double threshold = 0.85);
    AudioDescriptorExtractor* clone() const;
};
